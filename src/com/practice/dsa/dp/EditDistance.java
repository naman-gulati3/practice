...
//     5, 0, 0, 0
  ...
  private static int solve(String word1, int i, String word2, int j, int[][] dp) {
    ...
    // dp[i][j - 1] means insertion, we took a character from word2, i stays where it was in word1 and j moves to left in word2
    // dp[i - 1][j - 1] means replace
    dp[i][j] = 1 + Math.min(solve(word1, i - 1, word2, j, dp),
