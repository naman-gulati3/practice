...
public class SocialMediaServiceImpl implements SocialMediaService {
  ...
  public SocialMediaServiceImpl(
      UserRepository userRepository, QuestionAnswerRepository questionAnswerRepository) {
  ...
  public List<Question> getFeed(User user, List<Topic> topicsToFilter) {
    ...
    if (user != null) {
      List<Topic> userSubscribedTopics =
          topicsToFilter.stream().filter(t -> user.getSubscribedTopics().contains(t)).toList();

      questions = this.questionAnswerRepository.getQuestionsByTopic(userSubscribedTopics);
    } else {
      questions = this.questionAnswerRepository.getQuestionsByTopic(topicsToFilter);
    }
