...
public class Sudoku {
  ...
  private static boolean isValid(char[][] board, int row, int col, char ch) {
    for (int i = 0; i < 9; i++) {
      ...
      // to find the following rows using index i find the first row from above formula and add i /
      // 3 for each iteration of i

      // if we are at 5,7 first column of 3x3 box can be found by 3 * (col / 3) i.e row number 6
      // to find the following columns using index i find the first column from above formula and
      // add i % 3 for each iteration of i

      // Note: 3 * (row / 3) and 3 * (col / 3) only helps to find the first row and first column of
      // bounding 3x3 box
